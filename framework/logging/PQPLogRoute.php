 ...
 */
class PQPLogRoute extends WebLogRoute{
	...
	public function processLogs($logs)	{
		if(/*isset($_REQUEST['ws'])*/!SKY_DEBUG || isset($_REQUEST['IsWsdlRequest']))
			return;
		$this->process($logs);
	}
	...
	protected function process($logs){
		foreach($logs as $log){
			switch ($log[1]) {
				case 'memory':
					...
					break;
				case \Sky\logging\Logger::LEVEL_PROFILE:
					$this->processProfileLog($log);
					break;
				case \Sky\logging\Logger::LEVEL_ERROR:
					$this->processErrorLog($log);
					break;
				case \Sky\logging\Logger::LEVEL_INFO:
				case \Sky\logging\Logger::LEVEL_WARNING:
				case \Sky\logging\Logger::LEVEL_TRACE:
				default:
		...
		$this->entries['memoryTotals']['used'] = $this->getReadableFileSize(\Sky\Sky::getLogger()->getMemoryUsage());
		$this->entries['speedTotals']['total'] = $this->getReadableTime(\Sky\Sky::getLogger()->getExecutionTime());
	...
	protected function processProfileLog($log){
		...
		if(!strncasecmp($message,'begin:',6)){
			...
			if(($last=array_pop($this->stack))!==null && $last[0]===$token)
			{
				...
				throw new \Exception("ProfileLogRoute found a mismatching code block '$token' and '$last[0]'. Make sure the calls to Sky::beginProfile() and Sky::endProfile() be properly nested.");
			}
		}
	...
	protected function render($view,$data)
	{
		if(/*isset($_REQUEST['ws'])*/!SKY_DEBUG || isset($_REQUEST['IsWsdlRequest']))
			return;
		extract($data);
		include( SKY_PATH.DIRECTORY_SEPARATOR.'views'.DIRECTORY_SEPARATOR.$view.'.php');
	}
